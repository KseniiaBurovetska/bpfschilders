 ...
 * Returns { copyright, items } where items preserves document order.
 * Each item has { type: 'link'|'cookie', text, href? }.
 *
...
function parseBlock(block) {
  ...
  rows.forEach((row) => {
    ...
    if (type === 'copyright') {
      ...
      if (text) items.push({ type: 'link', text, href });
    } else if (type === 'cookie') {
      const text = cells[1]?.textContent?.trim() || '';
      if (text) items.push({ type: 'cookie', text });
    }
  });

  return { copyright, items };
}
